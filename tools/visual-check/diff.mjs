...
const ignore = new Set(['margin-left', 'margin-right', 'margin-inline-start', 'margin-inline-end', 'transition', 'transition-property', 'transition-duration', 'transition-timing-function', 'transition-delay', 'transition-behavior']);
let total = 0;
for (const f of fs.readdirSync(a).filter(f => f.endsWith('.json')).sort()) {
  ...
  const cmp = (label, x = {}, y = {}) => {
    for (const path of new Set([...Object.keys(x), ...Object.keys(y)])) {
      if (!x[path] || !y[path]) { console.log(`${f} ${label} ${path}: element ${x[path] ? 'missing' : 'extra'}`); total++; continue; }
      const zero = e => /,0(\.0)?,|,0(\.0)?$/.test(e['@box']);
      if (zero(x[path]) && zero(y[path])) continue; // not rendered in either build
      for (const prop of new Set([...Object.keys(x[path]), ...Object.keys(y[path])])) {
        ...
        const norm = v => String(v).replace(/localhost:\d+/g, 'localhost').replace(/to right/g, '90deg').replace(/ 0%/g, ' 0px');
  ...
  for (const k of new Set([...Object.keys(A.hover || {}), ...Object.keys(B.hover || {})])) cmp(`hover#${k}`, A.hover?.[k], B.hover?.[k]);
  const png = f.replace('.json', '.png');
  const ae = spawnSync('compare', ['-metric', 'AE', `${a}/${png}`, `${b}/${png}`, 'null:']).stderr.toString();
  if (!ae.startsWith('0')) { console.log(`${png} pixels differ: ${ae.trim()}`); total++; }
}
